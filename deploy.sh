...
kubectl apply -f k8s
kubectl set image deployments/client-deployment server=lbakerjs/scaling-octo-garbanzo-client:$GIT_SHA
kubectl set image deployments/server-deployment server=lbakerjs/scaling-octo-garbanzo-server:$GIT_SHA
kubectl set image deployments/worker-deployment server=lbakerjs/scaling-octo-garbanzo-worker:$GIT_SHA